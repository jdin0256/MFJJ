...
// Get references to the new buttons from index.html
...
let lastX = 0;
let lastY = 0;
let strokeColor = 'black'; // Variable to hold the current color
...
eraseBtn.addEventListener('click', () => {
    strokeColor = 'white'; // Eraser is just drawing in white!
});

// --- Drawing Functions ---
// The function now accepts a color parameter
function draw(x0, y0, x1, y1, color) {
    context.beginPath();
    context.moveTo(x0, y0);
    context.lineTo(x1, y1);
    context.strokeStyle = color; // Use the color passed to the function
    context.lineWidth = 10;      // Increased line width for a better eraser
    context.stroke();
    context.closePath();
}
...
canvas.addEventListener('mousemove', (e) => {
    ...
    // Add the current color to the data sent to Firebase
    const drawData = {
        x0: lastX,
        y0: lastY,
        x1: currentX,
...
database.ref('drawings').on('child_added', (snapshot) => {
    ...
    // Use the color from the Firebase data to draw
